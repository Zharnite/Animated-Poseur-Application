...
const LayerCard = (props) => {
    ...
    const selectedLayerName = props.selectedLayerName;
    ...
    return (
      <div id="layer-card" class={classname} onClick={setSelectedLayer}>
        <span id="layer-card-name" onDoubleClick={(e) => showLayerNamefieldHelper(e,layer.index) }>
          { 
            diplayLayerNamefield && classname=="selected" ? 
            <input className="layer-card-namefield" type="text" value={selectedLayerName} 
            onChange={setSelectedLayerName}
            onKeyPress={(e)=>hideLayerNamefieldHelper(e.key)}/>
            : layer.layer_name
          }
        </span>
        <img  alt ="L"
